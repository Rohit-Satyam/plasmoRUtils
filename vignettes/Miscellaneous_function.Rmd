...
easymakeOrgdbVEuPath <- function(
  ...
  # Temporary files
  gff_tmp <- tempfile(fileext = ".gff")
  gaf_tmp <- tempfile(fileext = ".gaf.gz")

  # Clean up after function finishes
  on.exit(
    unlink(c(gff_tmp, gaf_tmp)),
    add = TRUE
  )
  ...
  httr2::request(gff) |>
    ...
    httr2::req_perform(path = gff_tmp)

  # Download GAF
  httr2::request(gaf) |>
    httr2::req_auth_bearer_token(api_key) |>
    httr2::req_timeout(timeout) |>
    httr2::req_perform(path = gaf_tmp)

  # Pass local files to easymakeOrgdb()
  easymakeOrgdb(
    gff = gff_tmp,
    gaf = gaf_tmp,
    ...
...
txdb<-easyTxDbmaker(
 ...
 fasta="ToxoDB-68_TgondiiME49_Genome.fasta",
 abbr="TgondiiME49",
...
# To calculate the effective length of the genes you can use the following function. Adding extra layer 
# of code to fetch the GFF directly from database
...
getEffLenVEuPath(
  "https://plasmodb.org/a/service/raw-files/release-71/Pfalciparum3D7/gff/data/PlasmoDB-71_Pfalciparum3D7.gff"
) |>
  head()
```
...
gene_info <- easyNOISeqAnnot(
 gff="ToxoDB-68_EpraecoxHoughton.gff",
 fasta = "ToxoDB-68_EpraecoxHoughton_Genome.fasta")
...
- Time course Microarray dataset of *P. falciparum* 3D7 from [@painter2018genome]: `data("painter2018")`
- Time course Bulk RNASeq (Single-end) of *P. falciparum* 3D7 from [@toenhake2018chromatin]: `data("toenhake2018")`
- Time course Bulk RNASeq (Paired-end) of *P. falciparum* 3D7 from [@Kucharski2020]: `data("kucharski2020")`
- Reanalyzed scRNAseq of *T. gondii* ME49 data from [@lou2024single]: `data("gubbels2024")`
- Reanalyzed scRNAseq of *T. gondii* ME49 data from [@xue2020single]: `data("boothroyd2020")`
- Reanalyzed Bulk RNASeq (Paired-end) of *P. falciparum* 3D7 from [@Wichers2019]: `data("wichers2019")`
...
easytopGOVEuPath <- function(
  ...
  # Pass local files to easymakeOrgdb()
  easytopGO(geneID = p_values,
            useGAF = TRUE,
            useBiomart = FALSE,
            gaf=gaf_tmp,
            bkggset = background.gset, 
            category = "BP", stats = "ks",...)
}


## Performing ORA
gores<- easytopGOVEuPath(gaf = "https://plasmodb.org/a/service/raw-files/release-71/Pfalciparum3D7/gaf/PlasmoDB-71_Pfalciparum3D7_GO.gaf")
